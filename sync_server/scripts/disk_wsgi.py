...
def application(environ, start_response):
    ...
    disk_uuid = base.split('.')[0]
    print >> stderr, 'disk uuid', disk_uuid
    connection = get_connection(int(environ['SERVER_PORT']))
    # TODO: handle exceptions:
    file_path = get_disk_path(connection, device_uuid, disk_uuid)
    print >> stderr, 'file path', file_path
    return serve_file(environ, start_response, file_path)

def get_disk_path(connection, device_uuid, disk_uuid):
    """Map disk_uuid to file_path"""
